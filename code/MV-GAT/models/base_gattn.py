# -*- coding:UTF-8 -*-
# 定义一个model所需要的基础：包括损失函数 metric 优化过程
...
class BaseGAttN:
    ...
    def training(self,loss, lr, l2_coef):
        ...
        # training op,防止L2loss优化目标不一致，仅用分类loss
...
##########################
    ...
    def masked_softmax_cross_entropy(self,logits, labels, mask):
        ...
        # loss = tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(logits=logits, labels=labels), axis=1)
    ...
    def multilabel_masked_sigmoid_cross_entropy(self,logits, labels, mask):
        """Softmax cross-entropy loss with masking."""
