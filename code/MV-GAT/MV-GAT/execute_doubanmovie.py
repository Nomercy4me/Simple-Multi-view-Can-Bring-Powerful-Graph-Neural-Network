...
with tf.Graph().as_default():
    ...
    # output result,可以打印输出结果，计算ranking指标
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(nb_epochs):
            ...
            while tr_step * batch_size < tr_size:
                train_output, _, loss_value_tr, acc_tr = sess.run([log_resh,train_op, loss, accuracy],
                    feed_dict={
            ...
            while vl_step * batch_size < vl_size:
                ...
                label_align = label_alignment(output=val_output, label=y_val, mask=val_mask, sample_num=node_num,
                                              class_num=class_num)
                MAP(label_align, val_mask, node_num, class_num)
