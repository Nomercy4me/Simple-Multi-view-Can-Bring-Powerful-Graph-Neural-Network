    ...
    def build(self):
        ...
        if(FLAGS.mlp_flag == 1):
            for k in range(FLAGS.K_sample_num):
                ...
                for i in range(tmp_layer_cnt):
                    hidden = self.layers[tmp_layer_cnt * k + i](self.activations[-1])
            ...
            if(FLAGS.K_sample_num == 1):
                ...
                if(FLAGS.resnet == 1):
                    tmp_tensor = FLAGS.alpha*self.activations[tmp_layer_cnt] + FLAGS.beta*self.activations[2]
                else:
                    tmp_tensor = self.activations[tmp_layer_cnt]
                for k in range(2, FLAGS.K_sample_num+1):
                    if(FLAGS.resnet == 1):
                        tmp_tensor = tf.add(tmp_tensor, FLAGS.alpha*self.activations[(tmp_layer_cnt+1) * k - 1] + FLAGS.beta*self.activations[(tmp_layer_cnt+1) * k - (tmp_layer_cnt-1)])
                    else:
                        tmp_tensor = tf.add(tmp_tensor, self.activations[tmp_layer_cnt * k - 1])
...
class GCN(Model):
    ...
    def _build(self):
        ...
        if (FLAGS.mlp_flag == 0):
            ...
            self.vars['weights_GClayer3'] = glorot([FLAGS.hidden2, FLAGS.hidden2], name='weights_GClayer3')
            self.vars['weights_GClayer4'] = glorot([FLAGS.hidden2, FLAGS.hidden2], name='weights_GClayer4')
            self.vars['weights_GClayer5'] = glorot([FLAGS.hidden2, FLAGS.hidden2], name='weights_GClayer5')
            self.vars['weights_GClayer6'] = glorot([FLAGS.hidden2, FLAGS.hidden2], name='weights_GClayer6')
            self.vars['weights_GClayer7'] = glorot([FLAGS.hidden2, FLAGS.hidden2], name='weights_GClayer7')
            self.vars['weights_GClayer8'] = glorot([FLAGS.hidden2, FLAGS.hidden2], name='weights_GClayer8')
        ...
        for k in range(FLAGS.K_sample_num):
            ...
            if(FLAGS.mlp_flag == 0):
                ...
                # 第二层是用relu还是不用激活函数，这个是超参数，基准数据集影响不大，douban数据集不用激活函数
                self.layers.append(GraphConvolution(k, weights=self.vars['weights_GClayer3'], input_dim=FLAGS.hidden2,
                                                    output_dim=FLAGS.hidden2,
                                                    placeholders=self.placeholders,
                                                    act=tf.nn.relu,
                                                    dropout=True,
                ...
                # 第二层是用relu还是不用激活函数，这个是超参数，基准数据集影响不大，douban数据集不用激活函数
                self.layers.append(GraphConvolution(k, weights=self.vars['weights_GClayer4'], input_dim=FLAGS.hidden2,
                                                    output_dim=FLAGS.hidden2,
                                                    placeholders=self.placeholders,
                                                    act=tf.nn.relu,
                                                    dropout=True,
                ...
                # 第二层是用relu还是不用激活函数，这个是超参数，基准数据集影响不大，douban数据集不用激活函数
                self.layers.append(GraphConvolution(k, weights=self.vars['weights_GClayer5'], input_dim=FLAGS.hidden2,
                                                    output_dim=FLAGS.hidden2,
                                                    placeholders=self.placeholders,
                                                    act=tf.nn.relu,
                                                    dropout=True,
                ...
                # 第二层是用relu还是不用激活函数，这个是超参数，基准数据集影响不大，douban数据集不用激活函数
                self.layers.append(GraphConvolution(k, weights=self.vars['weights_GClayer6'], input_dim=FLAGS.hidden2,
                                                    output_dim=FLAGS.hidden2,
                                                    placeholders=self.placeholders,
                                                    act=tf.nn.relu,
                                                    dropout=True,
                ...
                # 第二层是用relu还是不用激活函数，这个是超参数，基准数据集影响不大，douban数据集不用激活函数
                self.layers.append(GraphConvolution(k, weights=self.vars['weights_GClayer7'], input_dim=FLAGS.hidden2,
                                                    output_dim=FLAGS.hidden2,
                                                    placeholders=self.placeholders,
                                                    act=tf.nn.relu,
                                                    dropout=True,
                ...
                # 第二层是用relu还是不用激活函数，这个是超参数，基准数据集影响不大，douban数据集不用激活函数
                self.layers.append(GraphConvolution(k, weights=self.vars['weights_GClayer8'], input_dim=FLAGS.hidden2,
                                                    output_dim=FLAGS.hidden2,
                                                    placeholders=self.placeholders,
                                                    act=tf.nn.relu,
                                                    dropout=True,
